...
async function addDataToFirestore(name, email, password, phone, dob, city, occupation) {
  try {
    const docRef = await addDoc(collection(db, "person"), {
      name,
      email,
...
export default function RegisterUser() {
  ...
  return (
    ...
    <nav className="bg-white shadow-sm p-4 fixed w-full top-0 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center space-x-4">
            <button onClick={() => setShowDropdown(!showDropdown)} className="text-gray-600 hover:text-green-700">
              <LogIn className="w-6 h-6" />
          </button>
            <Link href="/profile" className="text-gray-600 hover:text-green-700">
              <User className="w-6 h-6" />
          ...
          {showDropdown && (
            <div className="absolute mt-2 bg-white shadow-md rounded-lg py-2 w-40">
              <button onClick={() => { setShowForm(true); setShowDropdown(false); }} className="block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-left">User</button>
              <button onClick={() => { setShowOrgOptions(true); setShowDropdown(false); }} className="block px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-left">Organisation</button>
        </div>
          )}
        <div className="space-x-6">
            <Link href="/" className="text-gray-600 hover:text-green-700">Home</Link>
